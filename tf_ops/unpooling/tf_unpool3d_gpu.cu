...
{
    mean_interpolate_forward<<<B,1024>>>(B, N, M, C, K, nnIndex, nnCount, input, output);
...
{
    mean_interpolate_backward<<<B,1024>>>(B, N, M, C, K, nnIndex, nnCount, gradOutput, gradInput);
}

void weightedInterpolateLauncher(int B, int N, int M, int C, int K, const int* nnIndex, const int* nnCount,
                                 const float* input, const float* weight, float* output)
{
    weighted_interpolate_forward<<<B,1024>>>(B, N, M, C, K, nnIndex, nnCount, input, weight, output);
}

void weightedInterpolateGradLauncher(int B, int N, int M, int C, int K, const int* nnIndex, const int* nnCount,
                                     const float* gradOutput, const float* weight, float* gradInput)
{
    weighted_interpolate_backward<<<B,1024>>>(B, N, M, C, K, nnIndex, nnCount, gradOutput, weight, gradInput);
}