...
REGISTER_OP("FuzzyDepthwiseConv3d")
    ...
    .Input("nn_index: int32")       // neighbor indices: batch * out_mpoint * nn_sample
    .Input("nn_count: int32")       // number of neighbors: batch * out_mpoint
    .Input("bin_index: int32")      // kernel bin indices: batch * out_mpoint * nn_sample * 8
    .Input("bin_coeff: float32")    // kernel bin coefficients: batch * out_mpoint * nn_sample * 8
    .Output("output: float32")      // batch * out_mpoint * out_channels  (out_channels = in_channels * channel_multiplier)
    .SetShapeFn([](::tensorflow::shape_inference::InferenceContext* c) {
        ::tensorflow::shape_inference::ShapeHandle filter;
        c->WithRank(c->input(1), 3, &filter);
...
void fuzzyDepthwiseConv3dLauncher(int B, int N, int M, int C, int r, int K,
                                  const int* nnIndex, const int* nnCount,
                                  const int* binIndex, const float* binCoeff,
                                  const float* input, const float* filter, float* output);
class FuzzyDepthwiseConv3dGpuOp : public OpKernel {
    public:
        ...
        void Compute(OpKernelContext* context) override {
            ...
            OP_REQUIRES(context, filter_tensor.shape().dim_size(1)==C, errors::InvalidArgument("Input Channel size error of the filter"));
            OP_REQUIRES(context, nn_index_tensor.dims()==3, errors::InvalidArgument("The rank of nn_index should be 3."));
            OP_REQUIRES(context, bin_index_tensor.dims()==4, errors::InvalidArgument("The rank of bin_index should be 4."));
            OP_REQUIRES(context, bin_coeff_tensor.dims()==4, errors::InvalidArgument("The rank of bin_coeff should be 4."));
            ...
            float* out = &(output_flat(0));
            cudaMemset(out,0,sizeof(float)*B*M*C*r);
            fuzzyDepthwiseConv3dLauncher(B, N, M, C, r, K, nnIndex, nnCount, binIndex, binCoeff, in, filt, out);
        }
};
REGISTER_KERNEL_BUILDER(Name("FuzzyDepthwiseConv3d").Device(DEVICE_GPU), FuzzyDepthwiseConv3dGpuOp);


void fuzzyDepthwiseConv3dGradLauncher(int B, int N, int M, int F, int C, int r, int K,
                                 const int* nnIndex, const int* nnCount, const int* binIndex,
...
class FuzzyDepthwiseConv3dGradGpuOp : public OpKernel {
    public:
        ...
        void Compute(OpKernelContext* context) override {
            ...
            cudaMemset(gradIn,0,sizeof(float)*B*N*C);
            cudaMemset(gradFilt,0,sizeof(float)*F*C*r);
            fuzzyDepthwiseConv3dGradLauncher(B, N, M, F, C, r, K, nnIndex, nnCount, binIndex,
