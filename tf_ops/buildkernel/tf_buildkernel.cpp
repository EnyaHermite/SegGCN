...
REGISTER_OP("FuzzySphericalKernel")
    ...
    .Input("nn_count: int32")       // number of neighbors: batch * mpoint
    .Input("nn_dist: float32")      // distance to the neighbors: batch * mpoint * nn_sample
    .Output("filt_index: int32")    // kernel bin indices: batch * mpoint * nn_sample * 2
    .Output("filt_coeff: float32")  // kernel bin coefficients: batch * mpoint * nn_sample * 2
    .SetShapeFn([](::tensorflow::shape_inference::InferenceContext* c) {
        ...
        ::tensorflow::shape_inference::ShapeHandle filt_coeff = c->MakeShape({c->Dim(nn_index, 0),
                                                                              c->Dim(nn_index, 1),
                                                                              c->Dim(nn_index, 2),
                                                                              4});
...
class FuzzySphericalKernelGpuOp : public OpKernel {
    public:
        ...
        void Compute(OpKernelContext* context) override {
            ...
            OP_REQUIRES_OK(context, context->allocate_output(0, TensorShape{B,M,K,4}, &filt_index_tensor));
            OP_REQUIRES_OK(context, context->allocate_output(1, TensorShape{B,M,K,4}, &filt_coeff_tensor));
            auto filt_index_flat = filt_index_tensor->flat<int32>();
            ...
            cudaMemset(filtIndex, 0, sizeof(int)*B*M*K*4);
            cudaMemset(filtCoeff, 0, sizeof(float)*B*M*K*4);

            fuzzySphericalKernelLauncher(B, N, M, K, n_, p_, q_, radius_,
                                        database, query, nnIndex, nnCount, nnDist,
                                        filtIndex, filtCoeff);
        }
    private:
...
REGISTER_OP("KpconvKernel")
    .Attr("sigma: float")         // control radius range
    .Input("database: float32")     // database points: batch * npoint * 3 (x,y,z)
    .Input("query: float32")        // query points: batch * mpoint * 3
    .Input("kernel: float32")       // kernel points: num_kernel_points * 3
    .Input("nn_index: int32")       // neighbor and kernel bin indices: batch * mpoint * nn_sample
    .Input("nn_count: int32")       // number of neighbors: batch * mpoint
    .Output("filt_index: int32")    // kernel bin indices: batch * mpoint * nn_sample
    .SetShapeFn([](::tensorflow::shape_inference::InferenceContext* c) {
        ::tensorflow::shape_inference::ShapeHandle nn_index; // batch * mpoint * nn_sample
        c->WithRank(c->input(3), 3, &nn_index);

        c->set_output(0, nn_index);
        return Status::OK();
    });
REGISTER_OP("FuzzyKpconvKernel")
    .Attr("sigma: float")         // control radius range
    .Input("database: float32")     // database points: batch * npoint * 3 (x,y,z)
    .Input("query: float32")        // query points: batch * mpoint * 3
    .Input("kernel: float32")       // kernel points: num_kernel_points * 3
    .Input("nn_index: int32")       // neighbor and kernel bin indices: batch * mpoint * nn_sample
    .Input("nn_count: int32")       // number of neighbors: batch * mpoint
    .Output("filt_index: int32")    // kernel bin indices: batch * mpoint * nn_sample * 4
    .Output("filt_coeff: float32")  // kernel bin coefficients: batch * mpoint * nn_sample * 4
    .SetShapeFn([](::tensorflow::shape_inference::InferenceContext* c) {
        ::tensorflow::shape_inference::ShapeHandle nn_index; // batch * mpoint * nn_sample
        c->WithRank(c->input(3), 3, &nn_index);
...
class KpconvKernelGpuOp : public OpKernel {
    public:
        explicit KpconvKernelGpuOp(OpKernelConstruction* context) : OpKernel(context)
        {
            OP_REQUIRES_OK(context, context->GetAttr("sigma", &sigma_));
            OP_REQUIRES(context, sigma_ > 0, errors::InvalidArgument("control distance range sigma>0, got ", sigma_));
        }
        ...
        void Compute(OpKernelContext* context) override {
            ...
            int N = database_tensor.shape().dim_size(1); // number of database points
            int M = query_tensor.shape().dim_size(1);    // number of query points
            int T = kernel_tensor.shape().dim_size(0);   // number of kernel points
            int K = nn_index_tensor.shape().dim_size(2);

            OP_REQUIRES(context, database_tensor.dims()==3 && database_tensor.shape().dim_size(2)==3, errors::InvalidArgument("Shape of database points requires to be (batch, npoint, 3)"));
            OP_REQUIRES(context, query_tensor.dims()==3 && query_tensor.shape().dim_size(2)==3, errors::InvalidArgument("Shape of query points requires to be (batch, mpoint, 3)"));
            OP_REQUIRES(context, kernel_tensor.dims()==2 && kernel_tensor.shape().dim_size(1)==3, errors::InvalidArgument("Shape of kernel points requires to be (tpoint, 3)"));
            OP_REQUIRES(context, nn_index_tensor.dims()==3, errors::InvalidArgument("Shape of nn_index requires to be of rank 3"));
            ...
            kpconvKernelLauncher(B, N, M, T, K, sigma_, database, query,
                                 kernel, nnIndex, nnCount, filtIndex);
        }
    private:
        float sigma_;
};
REGISTER_KERNEL_BUILDER(Name("KpconvKernel").Device(DEVICE_GPU), KpconvKernelGpuOp);
...
class FuzzyKpconvKernelGpuOp : public OpKernel {
    public:
        explicit FuzzyKpconvKernelGpuOp(OpKernelConstruction* context) : OpKernel(context)
        {
            OP_REQUIRES_OK(context, context->GetAttr("sigma", &sigma_));
            OP_REQUIRES(context, sigma_ > 0, errors::InvalidArgument("control distance range sigma>0, got ", sigma_));
        }
        ...
        void Compute(OpKernelContext* context) override {
            ...
            fuzzyKpconvKernelLauncher(B, N, M, T, K, sigma_, database, query,
                                      kernel, nnIndex, nnCount, filtIndex, filtCoeff);
        }
    private:
        float sigma_;
};
REGISTER_KERNEL_BUILDER(Name("FuzzyKpconvKernel").Device(DEVICE_GPU), FuzzyKpconvKernelGpuOp);
