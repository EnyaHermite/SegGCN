...
def flip_point_cloud(batch_data):
    ...
    for k in range(batch_data.shape[0]):
        if np.random.rand()<0.5: # flip x-direction
            flipped_data[k,:,0] = -batch_data[k,:,0]
            flipped_data[k,:,1] =  batch_data[k,:,1]
        else:                     # flip y-direction
            flipped_data[k,:,0] =  batch_data[k,:,0]
            flipped_data[k,:,1] = -batch_data[k,:,1]
        flipped_data[k,:,2] = batch_data[k,:,2]
    return flipped_data


def jitter_point_color(batch_color, sigma=0.01, clip=0.02):
    """ Randomly jitter points. jittering is per point.
        Input:
          BxNx3 array, original batch of point clouds
        Return:
          BxNx3 array, jittered batch of point clouds
    """
    B, N, C = batch_color.shape
    assert(clip > 0)
    jittered_color = np.clip(sigma * np.random.randn(B, N, C), -1*clip, clip)
    jittered_color += batch_color
    jittered_color = np.clip(jittered_color, -1, 1)
