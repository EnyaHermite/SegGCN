...
from tf_unpool3d import weighted_interpolate
from tf_nnquery import build_nearest_neighbor

def check_weighted_interpolate(input, weight, nn_index, nn_count):
    B, M, C = input.shape
    ...
    for b in range(B):
        for n in range(N):
            ...
            for c in range(C):
                for k in range(K):
                    m = nn_index[b,n,k]
                    output[b, n, c] += input[b, m, c]*weight[b,n,k]

    return output

def check_weighted_interpolate_grad(input, grad_output, weight, nn_index, nn_count):
    ...
    for b in range(B):
        for n in range(N):
            ...
            for c in range(C):
                for k in range(K):
                    m = nn_index[b,n,k]
                    grad_input[b,m,c] += grad_output[b,n,c]*weight[b,n,k]
    return grad_input
...
if __name__=='__main__':
    ...
    # np.random.seed(100)
    B, N, M, C, radius, nnsample = (32, 128, 48, 64, 0.2, 1)
    kernel = [8, 2, 3]
    ...
    now = time.time()
    feat_out_cpu = check_weighted_interpolate(tmp3, tmp4, unpoolIndex, unpoolCount)
    grad_input_cpu = check_weighted_interpolate_grad(tmp3, tmp2, tmp4, unpoolIndex, unpoolCount)
    print(time.time() - now, cpp_gpu_time)

    if np.allclose(feat_out_cpu, featOut) and np.allclose(grad_input_cpu, gradIn):
        print(colored("weighted-interpolation test passed!",'green'))
