...
# sys.path.append(os.path.join(ROOT_DIR, 'tf_ops/weight_helper'))
...
def build_stride_graph(xyz, radius, nn_uplimit, num_sample, sample_method):
    ...
    query = tf.gather_nd(xyz, indices)
    inter_idx, inter_cnt, inter_dst = neighbor_fn(xyz, query, radius=radius,
                                                  nnsample=nn_uplimit)
    return inter_idx, inter_cnt, inter_dst, query
...
def unpool3d(inputs, nn_index, nn_count, nn_dist, scope, method):
    ...
    Returns:
        Variable tensor
    """
    print('unsampling info for debug:',nn_index, nn_dist)
    with tf.variable_scope(scope) as sc:
        if method == 'mean':
            outputs = tf_unpool3d.mean_interpolate(inputs, nn_index)
        elif method == 'weighted':
            weight = dist2weight(nn_dist)
            outputs = tf_unpool3d.weighted_interpolate(inputs, weight, nn_index)
        else:
