...
parser.add_argument('--num_input', type=int, default=8192, help='num_input [default: num_input 8192]')
parser.add_argument('--model', default='SPH3D_s3dis_resnet_fuzzy', help='Model name [default: SPH3D_s3dis]')
parser.add_argument('--config', default='s3dis_config_resnet', help='Model name [default: s3dis_config]')
...
LOG_FOUT = open(os.path.join(LOG_DIR,'log_evaluate_%s.txt'%FLAGS.model_name.split('-')[1]), 'a+')
...
net_config.num_input = FLAGS.num_input

resultsFolder = 'results-%d-%s'%(net_config.num_input,FLAGS.model_name.split('-')[1])
...
def eval_one_epoch(sess, ops, next_test_element):
    ...
    while True:
        try:
            ...
            while any(batch_inner_covered<batch_inner_size):
                ...
                for augType in ['none']:
                    if augType is 'aug':
                        cur_batch_input[0:bsize, ...] = augment_fn(batch_input)
                    else:
    ...
    log_string('eval mean loss: %f'%(loss_sum/(batch_idx+1e-10)))
    ...
    log_string("testing one batch require %.2f milliseconds"%(1000*test_time/(batch_idx+1e-10)))
