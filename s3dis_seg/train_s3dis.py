...
parser.add_argument('--model', default='SPH3D_s3dis_resnet_fuzzy', help='Model name [default: SPH3D_s3dis_resnet_fuzzy]')
parser.add_argument('--config', default='s3dis_config_resnet', help='Model name [default: s3dis_config_resnet]')
...
parser.add_argument('--decay_step', type=int, default=250000, help='Decay step for lr decay [default: 120000]')
...
os.system('export CUDA_VISIBLE_DEVICES=%d'%GPU_INDEX)
...
# LOG_DIR = os.path.join(rootDir,'log_s3dis_Area_%d_%s'%(FLAGS.test_area,FLAGS.log_dir))
LOG_DIR = os.path.join(rootDir,'log_s3dis_Area_%d_withDecay'%(FLAGS.test_area))
...
if 'kpconv' in MODEL_FILE.lower():
    net_config.kernel_points = np.loadtxt(os.path.join(LOG_DIR,'kernel_pts.txt'),dtype=np.float32,delimiter=',')
# np.savetxt(os.path.join(LOG_DIR,'kernel_pts.txt'),net_config.kernel_points,delimiter=',')

dataDir = os.path.join(rootDir, 'data/s3dis_3cm_overlap')
trainlist = [line.rstrip() for line in open(os.path.join(dataDir, 'train_files_fold%d.txt'%FLAGS.test_area))]
testlist =  [line.rstrip() for line in open(os.path.join(dataDir, 'test_files_fold%d.txt'%FLAGS.test_area))]
...
def augment_fn(batch_input, batch_label, batch_inner):
    ...
    # print(batch_input[0, 0, :], batch_input[-1, 0, :])
    ...
    # color augmentation
    # augment_rgb = batch_input[0:2*augSize,:,3:]
    # augment_rgb = data_util.jitter_point_color(augment_rgb)
    # batch_input[0:2*augSize,:,3:] = augment_rgb
    # print(batch_input[0,0,:],batch_input[-1,0,:])
...
def parse_fn(item):
    ...
    # all_in_one = tf.concat((xyz, rel_xyz, rgb, tf.cast(seg_label,tf.float32),
    #                         tf.cast(inner_label,tf.float32)), axis=-1)
...
def input_fn(filelist, batch_size=16, buffer_size=10000):
    ...
    # dataset = dataset.padded_batch(batch_size=1, drop_remainder=False)
